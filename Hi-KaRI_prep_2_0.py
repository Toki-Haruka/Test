...
def EachReading(eventalign_filepath,out_paths,chunk_size,Hajime,Owari,ColNames,locks):
   
    # header
    eventalign_file = open(eventalign_filepath,'r')
    eventalign_file.seek(Hajime)
    Reststr = eventalign_file.readline()
    rest = len(Reststr)
    pos_start = Hajime + rest
    chunk_split = None
    index_features = ['contig','read_index','line_length','namae','position']

    while pos_start < Owari:
        Hikari = 0
        chunkstr = ""
        lines = []
        Commencer = pos_start
        while Hikari < chunk_size:
            ThisLine = eventalign_file.readline()
            chunkstr += ThisLine
            Hikari += 1
            ThisLen = len(ThisLine)
            lines.append(ThisLen)
            Commencer += ThisLen
            if Commencer >=Owari:
                break
        chunkS = StringIO(chunkstr)
        chunk = pd.read_csv(chunkS, sep='\t', header=None)
        ...
        if len(lines) == chunk_size:
            ...
            index(chunk_concat[index_features],pos_start,out_paths,locks)
            pos_start += sum(lines)
            chunk_split = chunk[chunk['read_index'] == chunk.iloc[-1]['read_index']]
        else:
            chunk_concat = pd.concat([chunk_split,chunk])
            chunk_concat_size = len(chunk_concat.index)
            chunk_concat = chunk_concat.iloc[:len(lines)]
            chunk_concat['line_length'] = np.array(lines)
            chunk_concat["namae"] = chunk_concat["position"].apply(nearest_100K)
            index(chunk_concat[index_features],pos_start,out_paths,locks)
            pos_start += sum(lines)
...
##### Create and start consumers.
    ...
    consumers = [Consumer(task_queue=task_queue,task_function=EachReading,locks=locks) for i in range(n_processes)]#改回·删了helper
    
    for p in consumers:
        p.start()
        
##### Load tasks into task_queue. A task is eventalign information of one read.
    ColFile = open("../../Downloads/04.Pore-C/Hi-KaRI/Native_10M.tsv","r")
    ColStr = ColFile.readline()
    ColFile.close()
    ColNames = ColStr.rstrip().split("\t")
    PosStarts = list(range(0,10))
    PosEnds = list(range(1,11))
    file_size = os.path.getsize(eventalign_filepath)
    ##print(file_size)
    OneChunk = file_size // 10
    Hikari = 0
    for PosStart,PosEnd in zip(PosStarts,PosEnds):
        Hikari += 1
        Hajime = OneChunk * PosStart
        Owari = OneChunk * PosEnd
        if Hikari == 10:
            Owari = file_size
        ##print(f"{Hajime},{Owari}\n" )
        task_queue.put((eventalign_filepath,out_paths,chunk_size,Hajime,Owari,ColNames))
