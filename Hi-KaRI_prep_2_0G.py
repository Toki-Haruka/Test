...
def get_ids(f_index,data_info): #todo
    ...
    for condition_name, run_names in data_info.items():
        ...
        df_list += [pd.DataFrame({'ids':list(ids),condition_name:[1]*len(ids)})]

    df_merged = reduce(lambda  left,right: pd.merge(left,right,on=['ids'], how='outer'), df_list).fillna(0).set_index('ids')
